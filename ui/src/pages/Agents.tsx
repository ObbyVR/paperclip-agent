...
export function Agents() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("agent.title") }]);
  }, [setBreadcrumbs]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={t("agent.selectCompany")} />;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{t("agent.agentCount", { count: filtered.length })}</p>
      )}
      ...
      {agents && agents.length === 0 && (
        <EmptyState
          icon={Bot}
          message={t("agent.createFirst")}
          action={t("agent.newAgent")}
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        {t("agent.live", "Live")}{liveCount > 1 ? ` (${liveCount})` : ""}
      </span>
    </Link>
  );
}

  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">Seleziona un'azienda per visualizzare questa pagina.</p>
      </div>
    );
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          Piu' plugin dichiarano la route <code>{pluginRoutePath}</code>. Usa la route con plugin-id fino alla risoluzione del conflitto.
        </div>
      );
    }
  }

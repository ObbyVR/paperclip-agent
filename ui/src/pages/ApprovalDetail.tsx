...
import { ApprovalOutputPreview } from "../components/ApprovalOutputPreview";
...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("approval.title"), href: "/approvals" },
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? "—" },
    ]);
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Errore nel rifiuto"),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Errore nella richiesta di revisione"),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Errore nel reinvio"),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Errore nel commento"),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "Errore nell'eliminazione"),
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        {/* Output Preview — shows HTML/email/audit previews from linked issues */}
        {linkedIssues && linkedIssues.length > 0 && resolvedCompanyId && (
          <div className="pt-3 border-t border-border/60">
            <ApprovalOutputPreview linkedIssues={linkedIssues} companyId={resolvedCompanyId} />
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("approval.deleteAgentConfirm"))) return;
